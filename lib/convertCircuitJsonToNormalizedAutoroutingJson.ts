...
type NormalizedAutoroutingJson = {
  ...
  sorted_normalized_traces: Array<{
    net: number
    route: Array<{
      x: string
      y: string
...
export const convertCircuitJsonToNormalizedAutoroutingJson = (
  ...
  const tempTraces = circuitJson.filter(el => 
    ...
    const ports = trace.connected_source_port_ids.flatMap(portId => 
      circuitJson.filter(el => 
        el.type === "pcb_port" && el.source_port_id === portId
      )
    )
  ...
  const allPoints = tempTraces.flatMap(t => t.route.map(p => ({x: p.x, y: p.y})))
  ...
  const obstacles = circuitJson.filter(el => 
    ...
    const base = {
      ...
      net: connectivityMap.getNetConnectedToId(el.pcb_port_id || el.pcb_plated_hole_id)
    }
    ...
    return {
      ...base,
      type: "hole" as const,
      radius: (el.shape === "circle" ? el.outer_diameter/2 : Math.max(el.outer_width, el.outer_height)/2).toFixed(2)
    }
  })

  // Collect traces to route
  const traces = circuitJson.filter(el => 
    el.type === "source_trace"
  ).map(trace => {
    const net = connectivityMap.getNetConnectedToId(trace.source_trace_id)
    const ports = trace.connected_source_port_ids.flatMap(portId => 
  ...
  // Convert net IDs to numeric indices
  const allNets = Array.from(new Set([
    ...obstacles.map(o => o.net),
    ...traces.map(t => t.net)
  ].filter(n => n !== null))).sort() as string[]
  
  const netToIndex = new Map(allNets.map((net, i) => [net, i + 1]))
  ...
  const normalizedTraces = traces.map(t => ({
    ...t,
    net: netToIndex.get(t.net)!
  }))

  // Get unique numeric nets to route
  const netsToRoute = Array.from(new Set(normalizedTraces.map(t => t.net))).sort()

  return {
    allowed_layers: 1,
    nets_to_route: netsToRoute,
    sorted_normalized_objects: normalizedObstacles,
    sorted_normalized_traces: normalizedTraces
  }
}
